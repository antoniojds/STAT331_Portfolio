...
```{r}
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length, group = species)) + 
  geom_jitter( alpha = .1) + 
  facet_wrap(~species)
```
...
ggplot(data = surveys,mapping = aes(y = weight, xlab = "Weight", x = species, ylab = "Species")) + 
  geom_boxplot(alpha = .5)+
  labs(y = "Species Type", x = "Weight") + 
